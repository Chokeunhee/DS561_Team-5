    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": f"다음 정보를 바탕으로 도보 경로 설명을 작성해주세요: {tmap_result}."}
            ],
            max_tokens=1000
        )
...
def get_optimal_route(api_key,query: str):
    ...
    T_total_time = tmap_result['features'][0]['properties']['totalTime'] // 60
    T_total_distance = tmap_result['features'][0]['properties']['totalDistance']

    #result['features'][0]['properties']['totalTime'] <= 15 * 60

    #if tmap_result:
    if tmap_result['features'][0]['properties']['totalTime'] <= 15*60:
        ...
        return tmap_response, route_q, start, dest, T_total_time, T_total_distance, T_map # 텍스트와 이미지를 조합해 사용자에게 응답
    ...
    if google_result:
        ...
        return google_response, route_q, start, dest, T_total_time, T_total_distance, T_map
